...
__global__ void calc_eta_x_3D(Real *dev_bounds_L, Real *dev_bounds_R, Real *eta_x, int nx, int ny, int nz, int n_ghost, Real gamma, int n_fields)
{
  ...
  if (xid > n_ghost-2 && xid < nx-n_ghost && yid > n_ghost-2 && yid < ny-n_ghost+1 && zid > n_ghost-2 && zid < nz-n_ghost+1)
  {
    ...
    #ifdef DE
    d  =  dev_bounds_L[            id];
    vx =  dev_bounds_L[1*n_cells + id] / d;
    vy =  dev_bounds_L[2*n_cells + id] / d;
    vz =  dev_bounds_L[3*n_cells + id] / d;
    E = dev_bounds_L[4*n_cells + id];
    GE = fmin(dev_bounds_L[(n_fields-1)*n_cells + id], 1e-6);
    Ekin = 0.5 * d * ( vx*vx + vy*vy + vz*vz );
    pl = Get_Pressure_Dual_Energy( E, E - Ekin, GE, gamma ); 
    #else
    ...
    #ifdef DE
    d  =  dev_bounds_R[            id];
    vx =  dev_bounds_R[1*n_cells + id] / d;
    vy =  dev_bounds_R[2*n_cells + id] / d;
    vz =  dev_bounds_R[3*n_cells + id] / d;
    E = dev_bounds_R[4*n_cells + id];
    GE = fmin(dev_bounds_R[(n_fields-1)*n_cells + id], 1e-6);
    Ekin = 0.5 * d * ( vx*vx + vy*vy + vz*vz );
    pr = Get_Pressure_Dual_Energy( E, E - Ekin, GE, gamma );
    #else
