...
FLAGS = $(CUDA) $(PRECISION) $(OUTPUT) $(RECONSTRUCTION) $(SOLVER) $(INTEGRATOR) $(H_CORRECTION) $(COOLING) $(DUAL_ENERGY) $(GRAVITY) $(GRAVITY_RIEMANN) $(POTENTIAL) $(OUTPUT_POTENTIAL) $(OUTPUT_GRAVITY_DENSITY) $(PARTICLES)  $(SINGLE_PARTICLE_MASS) $(PARTICLE_INT) $(PARTICLE_IDS) $(PARTICLES_KDK) $(PECULIAR_VEL) $(PARTICLES_OMP) $(PARTICLES_CUDA) $(N_OMP_PARTICLE_THREADS) $(ONLY_PM) $(COSMOLOGY) $(GRACKLE_PRECISION) $(GRAVITY_CORRECTOR) $(SCALAR) $(REVERT_STEP) $(EXTRA_FIELD)#-DSTATIC_GRAV #-DDE -DSCALAR -DSLICES -DPROJECTION -DROTATED_PROJECTION
