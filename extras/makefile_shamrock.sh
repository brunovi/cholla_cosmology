...
OUTPUT_COOLING_RATE = -DOUTPUT_COOLING_RATE
SCALAR = -DSCALAR
...
FLAGS_HYDRO = $(CUDA) $(PRECISION) $(CPU_TIME) $(OUTPUT) $(RECONSTRUCTION) $(SOLVER) $(INTEGRATOR) $(H_CORRECTION)  $(DUAL_ENERGY)  $(SCALAR) $(REVERT_STEP) $(DENSITY_FLOOR) $(TEMPERATURE_FLOOR) $(DE_EKINETIC_LIMIT)
FLAGS_OMP = $(PARALLEL_OMP) $(N_OMP_THREADS)
FLAGS_GRAVITY = $(GRAVITY) $(GRAVITY_RIEMANN) $(POTENTIAL) $(OUTPUT_POTENTIAL) $(OUTPUT_GRAVITY_DENSITY) $(GRAVITY_CORRECTOR) $(EXTRA_FIELD) $(GRAVITY_CPU) $(OVERLAP_HYDRO_GRAV)
FLAGS_PARTICLES = $(PARTICLES) $(SINGLE_PARTICLE_MASS) $(PARTICLE_INT) $(PARTICLE_IDS) $(ONLY_PM)
FLAGS_COSMOLOGY = $(COSMOLOGY)
FLAGS_COOLING = $(COOLING) $(GRACKLE_PRECISION) $(OUTPUT_COOLING_RATE)
