...
FLAGS_HYDRO = $(CUDA) $(PRECISION) $(CPU_TIME) $(OUTPUT) $(RECONSTRUCTION) $(SOLVER) $(INTEGRATOR) $(H_CORRECTION)  $(DUAL_ENERGY)  $(SCALAR) $(REVERT_STEP) $(DENSITY_FLOOR) $(TEMPERATURE_FLOOR) $(DE_EKINETIC_LIMIT) $(CELL_SMOOTHING) $(ENERGY_FLOOR) $(SINGLE_ALLOC_GPU)
