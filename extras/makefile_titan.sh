...
FLAGS_HYDRO = $(CUDA) $(PRECISION) $(CPU_TIME) $(OUTPUT) $(RECONSTRUCTION) $(SOLVER) $(INTEGRATOR) $(H_CORRECTION) $(COOLING) $(DUAL_ENERGY) $(GRACKLE_PRECISION)  $(SCALAR) $(REVERT_STEP) $(DENSITY_FLOOR) $(DE_EKINETIC_LIMIT)
FLAGS_OMP = $(PARALLEL_OMP) $(N_OMP_THREADS)
FLAGS_GRAVITY = $(GRAVITY) $(GRAVITY_RIEMANN) $(POTENTIAL) $(OUTPUT_POTENTIAL) $(OUTPUT_GRAVITY_DENSITY) $(GRAVITY_CORRECTOR) $(EXTRA_FIELD) $(GRAVITY_CPU)
FLAGS_PARTICLES = $(PARTICLES) $(SINGLE_PARTICLE_MASS) $(PARTICLE_INT) $(PARTICLE_IDS) $(ONLY_PM)
FLAGS_COSMOLOGY = $(COSMOLOGY)
FLAGS = $(FLAGS_HYDRO) $(FLAGS_OMP) $(FLAGS_GRAVITY) $(FLAGS_PARTICLES) $(FLAGS_COSMOLOGY)  #-DSTATIC_GRAV #-DDE -DSCALAR -DSLICES -DPROJECTION -DROTATED_PROJECTION
