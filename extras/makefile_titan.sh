...
N_OMP_GRAVITY_THREADS = -DN_OMP_GRAVITY_THREADS=16
...
FLAGS_HYDRO = $(CUDA) $(PRECISION) $(CPU_TIME) $(OUTPUT) $(RECONSTRUCTION) $(SOLVER) $(INTEGRATOR) $(H_CORRECTION) $(COOLING) $(DUAL_ENERGY) $(GRACKLE_PRECISION)  $(SCALAR) $(REVERT_STEP)
FLAGS_GRAVITY = $(GRAVITY) $(GRAVITY_RIEMANN) $(POTENTIAL) $(OUTPUT_POTENTIAL) $(OUTPUT_GRAVITY_DENSITY) $(GRAVITY_OMP) $(N_OMP_GRAVITY_THREADS)
FLAGS_PARTICLES = $(PARTICLES) $(SINGLE_PARTICLE_MASS) $(PARTICLE_INT) $(PARTICLE_IDS) $(PARTICLES_KDK) $(PECULIAR_VEL) $(ONLY_PM) $(GRAVITY_CORRECTOR) $(EXTRA_FIELD)
FLAGS_COSMOLOGY = $(COSMOLOGY)
FLAGS = $(FLAGS_HYDRO) $(FLAGS_GRAVITY) $(FLAGS_PARTICLES) $(FLAGS_COSMOLOGY)  #-DSTATIC_GRAV #-DDE -DSCALAR -DSLICES -DPROJECTION -DROTATED_PROJECTION
CFLAGS 	  = $(OPTIMIZE) $(FLAGS) $(MPI_FLAGS) $(OMP_FLAGS)
CXXFLAGS  = $(OPTIMIZE) $(FLAGS) $(MPI_FLAGS) $(OMP_FLAGS)
NVCCFLAGS = $(FLAGS) -m64  -fmad=false -ccbin=$(CC) --expt-relaxed-constexpr
