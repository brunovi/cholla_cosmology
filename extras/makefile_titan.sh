...
FLAGS = $(CUDA) $(PRECISION) $(OUTPUT) $(RECONSTRUCTION) $(SOLVER) $(INTEGRATOR) $(H_CORRECTION) $(COOLING) $(DUAL_ENERGY) $(GRAVITY) $(GRAVITY_RIEMANN) $(POTENTIAL) $(OUTPUT_POTENTIAL) $(OUTPUT_GRAVITY_DENSITY) $(PARTICLES)  $(SINGLE_PARTICLE_MASS) $(PARTICLE_INT) $(PARTICLES_KDK) $(PECULIAR_VEL) $(GRAVITY_OMP) $(PARTICLES_CUDA) $(N_OMP_GRAVITY_THREADS) $(ONLY_PM) $(PARTICLE_IDS) $(COSMOLOGY)#-DSTATIC_GRAV #-DDE -DSCALAR -DSLICES -DPROJECTION -DROTATED_PROJECTION
