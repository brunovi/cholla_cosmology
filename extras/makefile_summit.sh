...
N_DEVICE_PER_NODE = -DN_DEVICE_PER_NODE=6
...
FLAGS_HYDRO = $(CUDA) $(PRECISION) $(CPU_TIME) $(OUTPUT) $(RECONSTRUCTION) $(SOLVER) $(INTEGRATOR) $(H_CORRECTION)  $(DUAL_ENERGY)  $(SCALAR) $(REVERT_STEP) $(DENSITY_FLOOR) $(TEMPERATURE_FLOOR) $(DE_EKINETIC_LIMIT) $(CELL_SMOOTHING) $(SINGLE_ALLOC_GPU) $(N_DEVICE_PER_NODE)
